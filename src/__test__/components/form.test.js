...
describe('<Form />', () => {
  it('it renders the <Form /> with populated data', () => {
    const { container, getByText, getByPlaceholderText } = render(
      <Form>
        ...
        <Form.Text>
          New to Netflix?
          <Form.Link to="/signup">Sign In Now.</Form.Link>
        </Form.Text>
        <Form.TextSmall>
  ...
  it('renders the <Form /> with an error', () => {
    const { container, getByText, queryByText } = render(
      <Form>
        <Form.Error>Your email address is aleady being used</Form.Error>
        <Form.Submit type="submit">Sign In</Form.Submit>
      </Form>
    );

    expect(getByText('Your email address is aleady being used')).toBeTruthy();
